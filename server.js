...
io.on('connection', (socket) => {
    ...
    socket.on('start-fleet', async ({ targetSize, autoWithdrawEnabled, withdrawLimit, mainWalletAddress: pWallet, defaultProxy, useFakeIp }) => {
        ...
        if (useFakeIp !== undefined) settings.useFakeIp = useFakeIp;
        fs.writeFileSync(SETTINGS_FILE, JSON.stringify(settings, null, 4));
...
function startRunner(acc, autoWithdraw, limit, mWallet) {
    ...
    const fakeIpFlag = settings.useFakeIp ? 'true' : 'false';

    const p = spawn('node', ['fast_tap.js', sArg, acc.proxy || '', addr, thr.toString(), rCode, seed, wAddr, sUrl, fakeIpFlag], { stdio: ['pipe', 'inherit', 'inherit', 'ipc'] });
    ...
    p.stderr.on('data', (d) => {
        ...
        const raw = d.toString();
        raw.split('\n').filter(Boolean).forEach(l => {
            pendingLogs.push({ name: acc.name, msg: '[ERR] ' + l.trim() });
            if (pendingLogs.length > 500) pendingLogs.shift();
        });
    });
...
server.on('error', (e) => {
    if (e.code === 'EADDRINUSE') {
        console.error(`[FATAL] Port ${PORT} already in use. Killing old processes...`);
