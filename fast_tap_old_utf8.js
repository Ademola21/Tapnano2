...
class FastTapper {
        ...
        this.ws.on('close', async () => {
            ...
            if (!this.halted) {
                if (this._rateLimited) {
                    ...
                    this.log('Rate limited ΓÇö signaling server to rotate ALL workers...', 'ALERT');
                    // Signal the server to rotate all workers to a new shared IP
                    try { process.send({ type: 'rate-limited' }); } catch (e) { }
                    // Don't reconnect here ΓÇö wait for the server to send 'rotate-proxy' IPC
                    ...
                    if (this.isPaused) {
                        this.log('Paused ΓÇö Reconnection loop suspended until Resume.');
                        return;
                    }
    ...
    async handleCaptchaRequired() {
        ...
        try {
            ...
            if (this.isPaused) {
                this.log('Paused ΓÇö Aborting CAPTCHA solve before Turnstile call.');
                return;
            }
